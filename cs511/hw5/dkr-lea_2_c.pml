...
byte I, nr_leaders;	/* will be used in init for assigning ids to nodes */

mtype = { one, two, winner };		/* symb. Msg. Names */
chan q[N] = [L] of { mtype, byte };	/* asynchronous channels */

ltl l1 { [] ((nr_leaders == 1) || (nr_leaders == 0)) }
...
{	
	...
	xs out;	/* channel assertions: exclusive send access to channel out */
...
init
{
	...
	atomic 
	{
		...
		:: I <= N ->
				...
				I++
		:: I > N ->		/* assigned all the numbers 1...N */
				break
		od;
